...
void LCD_Write_Command(int command) {
  IOWR_16DIRECT(LCD_0_BASE,0,command);
}

void LCD_Write_Data(int data) {
  IOWR_16DIRECT(LCD_0_BASE,2,data);
}
...
/* TP 01
volatile int TIMER_ID = 2;
volatile int counter = 0;

uint16_t TIMER_SETUP = 0x7;


void timer_irq(void* context, alt_u32 id){
	counter++;
	IOWR_32DIRECT(GPIO_0_BASE, 4, counter);
...
int main()
{
	...
	printf("TIMER_0_IRQ: %04X \n", TIMER_0_IRQ);
	printf("TIMER_0_BASE: %04X \n", TIMER_0_BASE);

	IOWR_32DIRECT(GPIO_0_BASE, 0, 0xFFFFFFFF);

	IOWR_32DIRECT(GPIO_0_BASE, 4, 0);

	alt_irq_register(TIMER_0_IRQ, (void*) TIMER_ID, (alt_isr_func) timer_irq);

	IOWR_16DIRECT(TIMER_0_BASE, 4, TIMER_SETUP);
